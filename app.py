...
# Konfigurasi halaman
st.set_page_config(
    page_title="Portal Kelas R7B", layout="wide"
)
...
st.markdown(
    ...
    /* Jadwal Harian */
    .schedule-day { 
        color: #B92B27; font-weight: 800; font-size: 1.4rem; 
    ...
    div[data-testid="stSelectbox"] label p {
        ...
        color: #D68A82 !important; /* Pakai warna salmon/peach biar estetik */
    }
    """,
...
# Header Estetik
st.markdown("<h1 class='main-title'>📚 Portal Kelas R7B</h1>", unsafe_allow_html=True)
st.markdown(
    "<h5 class='sub-title'>Ruang informasi terpadu kelompok, tugas, dan jadwal kuliah Mahasigma R7B</h5>",
    unsafe_allow_html=True,
)
...
# --- TAB 1: INFO KELOMPOK ---
with tab1:
  st.markdown("## Daftar Kelompok Mata Kuliah")
  ...
  try:
    ...
    pilih_mk = st.selectbox("Pilih Mata Kuliah:", df_kel[kolom_mk].unique(), key="mk_kelompok")
    ...
    for kel in df_filtered_kel[kolom_kelompok].unique():
      ...
      for index, row in df_anggota.reset_index(drop=True).iterrows():
        ...
        nama = row.iloc[3]
        card_html += (
            "<div style='display: flex; justify-content: space-between; align-items: center; padding-bottom: 8px; border-bottom: 1px dashed #B92B27;'>"
            f"<span style='color: #F5E6CC; font-size: 0.95rem;'><b>{index + 1}.</b> {nama}</span>"
            f"<span class='custom-badge'>{npm}</span>"
            "</div>"
        )
...
# --- TAB 2: DAFTAR TUGAS ---
with tab2:
  st.markdown("## List Tugas & Deadline")
  st.markdown(
      "<p style='color: #D68A82; font-size: 1.25rem; margin-bottom: 1.5rem;'>"
      "Semua tugas terangkum di sini. Pantau terus deadline-nya dan jangan lupa dikerjain!!</p>", 
      unsafe_allow_html=True
  )
  ...
  try:
    ...
    for index, row in df_tugas.iterrows():
      ...
      jenis_border = "#D68A82" if "kelompok" in str(jenis).lower() else "#B92B27"
      ...
      tugas_html = (
          ...
          "<div>"
          f"<h3 style='margin: 0; color: #F5E6CC !important; font-size: 1.25rem;'>{nama_tugas}</h3>"
          f"<span style='color: #D68A82; font-weight: 600; font-size: 0.95rem;'> Mata Kuliah : {matkul}</span>"
          "</div>"
          "<div style='text-align: right;'>"
          f"<span class='custom-badge' style='background: {jenis_bg}; color: {jenis_color}; border-color: {jenis_border}; margin-bottom: 5px;'>{jenis}</span><br>"
          f"<span class='custom-badge' style='background: #1D2B64; color: #D68A82; border-color: #B92B27;'>📄 {bentuk}</span>"
          "</div>"
          "</div>"
          "<hr style='border-color: #B92B27; margin: 0.8rem 0;'>"
          "<div style='display: grid; grid-template-columns: 1fr 1fr; gap: 12px; font-size: 0.9rem; color: #D68A82;'>"
          "<div>"
          "<span style='color: #D68A82; font-size: 0.85rem;'>Pertemuan / Tanggal:</span><br>"
          f"<b style='color: #F5E6CC;'>{pertemuan} — {tanggal}</b>"
          "</div>"
          "<div>"
          "<span style='color: #D68A82; font-size: 0.85rem;'>Tempat Pengumpul:</span><br>"
          f"<b style='color: #F5E6CC;'>{tempat}</b>"
          "</div>"
          "<div style='grid-column: span 2; background: rgba(185, 43, 39, 0.15); padding: 8px 12px; border-radius: 8px; border-left: 3px solid #B92B27;'>"
          "<span style='color: #B92B27; font-size: 0.85rem; font-weight: 700;'>⏳ DEADLINE:</span><br>"
          f"<b style='color: #F5E6CC; font-size: 1rem;'>{deadline}</b>"
          "</div>"
          "</div>"
          "</div>"
      )
...
# --- TAB 3: JADWAL KULIAH ---
with tab3:
  st.markdown("## Jadwal Kuliah Semester 7")
  
  st.markdown("<div class='schedule-day'>Senin</div>", unsafe_allow_html=True)
  senin_html = (
      "<div class='schedule-card'>"
      "<span class='schedule-time'>07:30 - 09:10</span> <span class='custom-badge' style='float: right;'>R.5.3-2</span>"
      "<div class='schedule-title'>Sistem Berbasis Pengetahuan (+)</div>"
      "<div class='schedule-dosen'>Dr. Alusyanti Primawati M.Kom.</div>"
      "<hr style='border-color: #B92B27; margin: 0.8rem 0;'>"
      "<span class='schedule-time'>09:10 - 10:50</span> <span class='custom-badge' style='float: right;'>R.5.3-2</span>"
  ...
  selasa_html = (
      "<div class='schedule-card'>"
      "<span class='schedule-time'>10:50 - 12:30</span> <span class='custom-badge' style='float: right;'>R.5.3-2</span>"
      "<div class='schedule-title'>E-Commerce (+) #)</div>"
      "<div class='schedule-dosen'>Rahmadanil M.Kom.</div>"
      "<hr style='border-color: #B92B27; margin: 0.8rem 0;'>"
      "<span class='schedule-time'>12:30 - 14:10</span> <span class='custom-badge' style='float: right;'>R.5.3-2</span>"
  ...
  rabu_html = (
      "<div class='schedule-card'>"
      "<span class='schedule-time'>07:30 - 09:40</span> <span class='custom-badge' style='float: right;'>R.5.3-2</span>"
      "<div class='schedule-title'>Filsafat Ilmu</div>"
      "<div class='schedule-dosen'>Drs. Surajiyo M.Si.</div>"
      "<hr style='border-color: #B92B27; margin: 0.8rem 0;'>"
      "<span class='schedule-time'>10:00 - 12:30</span> <span class='custom-badge' style='float: right;'>R.5.3-2</span>"
  ...
  kamis_html = (
      "<div class='schedule-card'>"
      "<span class='schedule-time'>07:00 - 09:30</span> <span class='custom-badge' style='float: right;'>R.5.3-2</span>"
      "<div class='schedule-title'>Riset Operasional</div>"
      "<div class='schedule-dosen'>Agung Ferdinan Sandy S.T., M.K...</div>"
      "<hr style='border-color: #B92B27; margin: 0.8rem 0;'>"
      "<span class='schedule-time'>09:30 - 12:00</span> <span class='custom-badge' style='float: right;'>R.5.3-2</span>"
...
# --- TAB 4: JADWAL LAB ---
with tab4:
  st.markdown("## Jadwal Praktikum / Lab (LKMM1)")
  st.markdown("<br>", unsafe_allow_html=True)

  lab1_html = (
      "<div class='group-card'>"
      "<h3>1. E-Commerce (+)</h3>"
      "<span class='custom-badge' style='margin-bottom: 10px;'>Ruang: LKMM1</span>"
      ...
      "<span style='color: #B92B27; font-weight: 600;'>Pra-UTS:</span> 15 Sept, 29 Sept, 13 Okt 2026<br>"
      "<span style='color: #B92B27; font-weight: 600;'>Pra-UAS:</span> 10 Nov, 24 Nov, 8 Des 2026"
      "</div>"
      "</div>"
  )
  ...
  lab2_html = (
      ...
      "<hr style='border-color: #B92B27; margin: 1rem 0;'>"
      "<div style='color: #D68A82; font-size: 0.9rem;'>"
      "<span style='color: #B92B27; font-weight: 600;'>Pra-UTS:</span> 24 Sept, 8 Okt, 22 Okt 2026<br>"
      "<span style='color: #B92B27; font-weight: 600;'>Pra-UAS:</span> 19 Nov, 3 Des, 17 Des 2026"
      "</div>"
      "</div>"
  )
